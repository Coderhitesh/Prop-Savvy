...
exports.createProperty = async (req, res) => {
    ...
    try {
        const { name, description, rating, completeAddress, startingPrice, location, propertyType, status, MetaTitle, MetaKeywords, MetaDescription } = req.body;
        // Validate request
...
exports.findAllProperties = async (req, res) => {
    try {
        const properties = await Property.find().populate('location').populate('propertyType');
...
exports.findOneProperty = async (req, res) => {
    try {
        ...
        const foundProperty = await Property.findById(id).populate('location').populate('propertyType');
        if (!foundProperty) {
            return res.status(404).json({
                success: false,
